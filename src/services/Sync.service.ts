...
function isLegacyMediaMigrationMarkedComplete(): boolean {
	return (
		localStorage.getItem(SETTINGS_STORAGE_KEYS.CLOUD_MEDIA_MIGRATION_V1_DONE) ===
		LEGACY_MEDIA_MIGRATION_COMPLETION_VALUE
	);
}
...
export async function setupSync(password: string): Promise<boolean> {
	...
	try {
		...
		const { error } = await supabase.from("user_sync_preferences").upsert({
			user_id: user.id,
			sync_enabled: true,
		...
		for (const image of message.generatedImages) {
			...
			if (image._thoughtSignatureRef) {
				...
				if (!resolvedThoughtSignature) {
					throw new Error(
						`Failed to materialize image thought signature blob ${image._thoughtSignatureRef.blobId}`
					);
				}
	...
	if (Array.isArray(message.parts)) {
		for (const part of message.parts) {
			if (part._thoughtSignatureRef) {
				...
				if (!resolvedThoughtSignature) {
					throw new Error(
						`Failed to materialize part thought signature blob ${part._thoughtSignatureRef.blobId}`
					);
				}
...
export async function fetchSyncedChatsMetadata(): Promise<DbChat[]> {
	...
	const results = await Promise.allSettled(
		activeRows.map(async (row) => {
			const plaintext = await crypto.decrypt(key, crypto.fromHex(row.encrypted_data), crypto.fromHex(row.iv));
...
export async function fetchSyncedChatMetadata(chatId: string): Promise<DbChat | null> {
	...
	try {
		const plaintext = await crypto.decrypt(key, crypto.fromHex(data.encrypted_data), crypto.fromHex(data.iv));
		const parsed = JSON.parse(plaintext);
...
function serializeChatMetadata(chat: DbChat, messageCountOverride?: number): string {
	const resolvedMessageCount =
		typeof messageCountOverride === "number" ? Math.max(0, messageCountOverride) : (chat.content?.length ?? 0);
...
async function serializeMessage(message: Message): Promise<{ json: string; uploadedBlobIds: string[] }> {
	...
	if (message.generatedImages && message.generatedImages.length > 0) {
		...
		if (thoughtSignatureUploadItems.length > 0) {
			try {
				const refs = await blobStore.uploadEncryptedBlobsBatch(
					thoughtSignatureUploadItems.map((item) => ({
						data: item.data,
				...
				console.warn(
					"serializeMessage: blob upload failed for generatedImage thought signatures, falling back to inline:",
					err
				);
			}
		}
	...
	const parts = await Promise.all(
		(message.parts || []).map(async (part: any) => {
			...
			if (preservedPartThoughtSignatureRef) {
				...
				if (sigBytes.byteLength > blobStore.BLOB_SIZE_THRESHOLD) {
					try {
						partThoughtSignatureRef = await blobStore.uploadEncryptedBlob(
							sigBytes,
						...
						console.warn(
							"serializeMessage: blob upload failed for part thoughtSignature, falling back to inline:",
							err
						);
					}
				}
...
function deserializeMessage(message: any): Message {
		...
		generatedImages = message.generatedImages.map((img: any) => {
			...
			return {
				...
				thoughtSignature: thoughtSignatureRef
					? undefined
					: typeof img.thoughtSignature === "string"
	...
	const parts = (message.parts || []).map((part: any) => {
		...
		const files = serializedAttachments
			.map((att: any) => {
...
export async function pushChat(chat: DbChat, messageCountOverride?: number): Promise<boolean> {
	...
	try {
		...
		const { error } = await supabase.from("user_synced_chats").upsert({
			user_id: user.id,
			id: chat.id,
...
async function pushChatMessagesRange(chat: DbChat, startInclusive: number, endExclusive: number): Promise<number> {
	...
	const flushRows = async (
		...
		const { error } = await supabase.from("user_synced_messages").upsert(rows);
...
function messageSignature(message: Message): string {
	return JSON.stringify({
		...
		generatedImages: (message.generatedImages || []).map((img) => ({
			...
			size: img.base64.length > 0 ? img.base64.length : (img._blobRef?.size ?? 0)
		})),
		parts: (message.parts || []).map((part) => ({
			...
			attachments: Array.from(part.attachments || []).map((file) => ({
				name: file.name,
				size: file.size > 0 ? file.size : ((file as any)?._blobRef?.size ?? 0),
				type: file.type,
				lastModified: file.lastModified
			}))
		}))
...
async function pushChatIncremental(previous: DbChat | undefined, current: DbChat): Promise<boolean> {
	...
	const metadataMessageCount = hasPotentiallyPartialSnapshot ? baselineCount : currentMessages.length;
...
export async function pushPersona(persona: DbPersonality): Promise<boolean> {
	...
	try {
		...
		const { error } = await supabase.from("user_synced_personas").upsert({
			user_id: user.id,
			id: persona.id,
			encrypted_data: crypto.toHex(ciphertext),
			iv: crypto.toHex(iv),
...
export async function pushSettings(settings: Record<string, string>): Promise<boolean> {
	...
	try {
		...
		const { error } = await supabase.from("user_synced_settings").upsert({
			user_id: user.id,
			encrypted_data: crypto.toHex(ciphertext),
			iv: crypto.toHex(iv)
		});
...
export async function pushAll(): Promise<void> {
	...
	try {
		...
		if (chatFailures > 0 || personaFailures > 0) {
			const detail = [
				chatFailures && `${chatFailures} chat(s)`,
				personaFailures && `${personaFailures} persona(s)`
			]
				.filter(Boolean)
				.join(", ");
...
function hasInlineMediaAboveThreshold(message: any): boolean {
	if (Array.isArray(message?.generatedImages)) {
		for (const img of message.generatedImages) {
			if (
				...
				estimateBase64ByteLength(img.base64) > blobStore.BLOB_SIZE_THRESHOLD
			) {
				return true;
			}
			if (
				typeof img?.thoughtSignature === "string" &&
				utf8Encode(img.thoughtSignature).byteLength > blobStore.BLOB_SIZE_THRESHOLD
			) {
				return true;
			}
		}
	...
	if (Array.isArray(message?.parts)) {
		for (const part of message.parts) {
			if (
				typeof part?.thoughtSignature === "string" &&
				utf8Encode(part.thoughtSignature).byteLength > blobStore.BLOB_SIZE_THRESHOLD
			) {
				return true;
			}
			...
			if (
				...
				estimateBase64ByteLength(part.inlineData.data) > blobStore.BLOB_SIZE_THRESHOLD
			) {
				return true;
			}
			...
			for (const att of part.attachments) {
				if (
					...
					estimateBase64ByteLength(att.base64) > blobStore.BLOB_SIZE_THRESHOLD
				) {
					return true;
				}
			}
...
async function pullChats(
	userId: string,
	key: CryptoKey,
	options?: { hydrateMessages?: boolean; materializeMedia?: boolean }
): Promise<void> {
				...
				if (remoteMessages !== null) {
					if (remoteMessages.length > 0) {
						content = options?.materializeMedia
							? await Promise.all(
									remoteMessages.map((message) => materializeMessageBlobBackedMedia(message))
								)
							: remoteMessages;
...
async function getRemoteMessageCount(
	chatId: string,
	userId: string,
		...
		const workers = Array.from({ length: workerCount }, async () => {
			while (true) {
				...
				try {
					...
					console.error(
						`fetchRemoteMessageRange: failed to decrypt chat=${chatId} idx=${row.message_index}`,
						err
					);
				}
			}
...
export async function fetchAllSyncedChatMessages(
	chatId: string,
	options?: { signal?: AbortSignal }
): Promise<Message[] | null> {
	if (!isSyncActive()) return null;
...
async function pullPersonas(userId: string, key: CryptoKey): Promise<void> {
	...
	for (const row of data) {
		if (row.deleted) {
			try {
				await db.personalities.delete(row.id);
			} catch {
				/* may not exist */
			}
			continue;
	...
	if (!prefs.syncEnabled) {
		...
		if (!hasSeenSyncPrompt()) {
			const hasEncryptionMaterial =
				!!prefs.encryptionSalt && !!prefs.keyVerification && !!prefs.keyVerificationIv;
