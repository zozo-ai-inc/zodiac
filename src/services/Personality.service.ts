...
import type { DbPersonality, Personality, PersonaSortMode, SyncInfo } from "../types/Personality";
import { v4 as uuidv4 } from "uuid";
import {
	getMarketplacePersonaVersion,
	getMarketplacePersonaVersions,
	fetchMarketplacePersona
...
async function fetchSyncedPersonasCached(force = false): Promise<DbPersonality[]> {
	...
	syncedPersonasFetchPromise = syncService
		.fetchSyncedPersonas()
		.then((remote) => {
...
export async function reloadFromDb() {
	...
	const syncedIds = personalitiesArray.filter((p) => p.syncedFrom).map((p) => p.syncedFrom!);

	const marketplaceVersions = syncedIds.length > 0 ? await getMarketplacePersonaVersions(syncedIds) : new Map();
		...
		if (lastId) {
			if (lastId === "-1") {
				...
				const input = document.querySelector<HTMLInputElement>(
					`#personality-${lastId} input[name='personality']`
				);
				if (input) {
					input.click();
				}
...
export async function add(personality: Personality, explicitId?: string): Promise<boolean> {
	...
	const id = marketplaceId ? marketplaceId : explicitId && explicitId !== "-1" ? explicitId : uuidv4();
...
export async function updateFromMarketplace(localId: string, marketplaceId: string): Promise<boolean> {
	...
	info({
		...
		text: `${marketplaceData.name} has been updated to version ${marketplaceData.version}.`
	});
	return true;
}
