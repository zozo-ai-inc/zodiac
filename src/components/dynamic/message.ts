...
function getRequestSlugs(debugInfo?: MessageDebugInfo): string[] {
	const slugs = [debugInfo?.requestSlug, ...(debugInfo?.requestSlugs ?? [])].filter(
...
async function getMessageDebugInfoForDisplay(message: Message, index: number): Promise<MessageDebugInfo | undefined> {
	const baseDebugInfo = message.debugInfo;
	if (!baseDebugInfo || message.role !== "user" || !baseDebugInfo.premiumEndpointEnabled) {
		return baseDebugInfo;
	}

	const existingSlugs = getRequestSlugs(baseDebugInfo);
	if (existingSlugs.length > 0) {
		return baseDebugInfo;
	}

	const currentChat = await chatsService.getCurrentChat();
	const persistedMessage = currentChat?.content?.[index];
	const persistedDebugInfo = persistedMessage?.debugInfo ?? baseDebugInfo;
	const persistedSlugs = getRequestSlugs(persistedDebugInfo);
	if (persistedSlugs.length > 0) {
		return {
			...persistedDebugInfo,
			requestSlug: persistedDebugInfo.requestSlug ?? persistedSlugs[0],
			requestSlugs: persistedSlugs
		};
	}
	...
	const downstreamSlugs: string[] = [];
	for (let nextIndex = index + 1; nextIndex < currentChat.content.length; nextIndex++) {
		const nextMessage = currentChat.content[nextIndex];
		if (!nextMessage) continue;
		if (!nextMessage.hidden && nextMessage.role === "user") {
			break;
		}

		downstreamSlugs.push(...getRequestSlugs(nextMessage.debugInfo));
	}

	const uniqueDownstreamSlugs = Array.from(new Set(downstreamSlugs));
	if (uniqueDownstreamSlugs.length === 0) {
		return persistedDebugInfo;
	}

	return {
		...persistedDebugInfo,
		requestSlug: uniqueDownstreamSlugs[0],
		requestSlugs: uniqueDownstreamSlugs
	};
}
...
export const messageElement = async (message: Message, index: number): Promise<HTMLElement> => {
	...
	const debugButtonMarkup = message.debugInfo
	...
	//model message
		...
		if (isNarrator) {
			...
			const narratorFooterMarkup =
				originModelLabel || debugButtonMarkup
					? `<div class="narrator-footer">` +
						`${originModelLabel ? `<div class="message-originator narrator-originator" title="${escapeHtml(message.originModel || originModelLabel)}">Generated by ${escapeHtml(originModelLabel)}</div>` : ""}` +
						`${debugButtonMarkup ? `<div class="narrator-debug-actions">${debugButtonMarkup}</div>` : ""}` +
						`</div>`
					: "";
			// Simplified narrator header - no pfp, no persona switching
			messageDiv.innerHTML = `<div class="message-header narrator-header">
			...
			messageDiv.innerHTML = `<div class="message-header">
                ...
                <div class="message-title">
                    ...
                    ${originModelLabel ? `<div class="message-originator" title="${escapeHtml(message.originModel || originModelLabel)}">Generated by ${escapeHtml(originModelLabel)}</div>` : ""}
                </div>
                <div class="message-actions">
                    ${debugButtonMarkup}
                    <button class="btn-edit btn-textual material-symbols-outlined">edit</button>
			...
			if (message.groundingContent) {
				const shadow = messageDiv
					.querySelector<HTMLElement>(".message-grounding-rendered-content")!
	...
	setupMessageDebug(messageDiv, message, index);
...
function setupMessageDebug(messageElement: HTMLElement, message: Message, index: number) {
	const debugButtons = messageElement.querySelectorAll<HTMLButtonElement>(".btn-message-debug");
...
function setupMessageEditing(messageElement: HTMLElement) {
	...
	const messageText =
		messageElement.querySelector<HTMLElement>(".message-text-content") ||
		messageElement.querySelector<HTMLElement>(".message-text");
		...
		() =>
			void (async () => {
				...
				messageText.innerText =
					parserService.parseHtmlToMarkdown(unwrapMentionsToRaw(messageText.innerHTML)) || ""; // Convert HTML to Markdown for editing
				messageText.focus();
...
function findPersonaInput(personalityId: string): HTMLInputElement | null {
	if (personalityId && personalityId !== "-1") {
		const existing = document.querySelector<HTMLInputElement>(
			`#personality-${personalityId} input[name='personality']`
		);
		if (existing) {
			return existing;
		}
...
function resolveBlobAttachmentPreviews(messageDiv: HTMLElement, message: Message): void {
	...
	pending.forEach(({ idx, imgEl, original }) => {
		resolveAttachmentFile(original)
			.then((resolved: File) => {
